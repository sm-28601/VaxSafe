export function findOptimalRoute(startNodeId, endNodeId, nodes, routes) {
  ...
  while (unvisited.size) {
    ...
    for (let neighbor of graph[current] || []) {
      const targetNode = nodes.find(n => n.id === neighbor.id);

      // Heavy penalty for critical nodes to force routing around them
      const isCritical = targetNode?.status === 'critical';

      // Lower reliability means higher risk multiplier
      const riskPenalty = targetNode?.reliability
        ? (1 - targetNode.reliability)
        : 0.5;
  ...
  // If path is just the endNode, it means no path was found
...
export function buildGraph(nodes, routes) {
  ...
  routes.forEach(route => {
    ...
    for (let i = 0; i < route.nodes.length - 1; i++) {
      ...
      if (!graph[from].find(e => e.id === to)) {
        graph[from].push({ id: to, distance: segmentDistance });
      }

      if (!graph[to]) graph[to] = [];
      if (!graph[to].find(e => e.id === from)) {
